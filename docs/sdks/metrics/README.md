...
async function run() {
  const result = await orq.deployments.metrics.create("<id>", {});
...
import { deploymentsMetricsCreate } from "orq-poc-typescript2/funcs/deploymentsMetricsCreate.js";
...
async function run() {
  const res = await deploymentsMetricsCreate(orq, "<id>", {});
