...
async function run() {
  const result = await orq.deployments.addMetrics("<id>", {});
...
async function run() {
  const res = await deploymentsAddMetrics(orq, "<id>", {});
...
**Promise\<[operations.DeploymentListResponseBody](../../models/operations/deploymentlistresponsebody.md)\>**
